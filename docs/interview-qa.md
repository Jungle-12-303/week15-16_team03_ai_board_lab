...
| `ddl-auto=update`는 실서비스에서 쓰면 안 되지 않나요? | 맞습니다. 로컬 개발 속도를 위해 사용했습니다. 배포 환경에서는 Flyway나 Liquibase로 migration을 관리하는 게 맞습니다. |
...
| 인증은 어떻게 처리했나요? | 로그인 성공 시 JWT를 발급하고, 프론트가 API 요청에 Authorization header로 전달합니다. Spring Security filter가 토큰을 검증해 인증 상태를 만듭니다. |
...
| 질문 | 좋은 답변 방향 |
|---|---|
| 다시 만든다면 뭐부터 고칠 건가요? | 평가셋 확장, migration 도입, refresh token 보안, embedding job retry, reranker 도입을 우선하겠습니다. |
...
| 다음 주에 하루 더 있다면 무엇을 하겠습니까? | RAG 결과 캡처 추가, holdout 평가셋 확장, MCP 도구 추가, JWT refresh token, migration 도입 중 하나를 우선하겠습니다. |
...
| `ddl-auto=update`는 왜 썼나요? | 로컬 개발 속도를 위해 사용했습니다. 운영에서는 Flyway/Liquibase로 migration을 관리하는 것이 맞습니다. |
