...
검색 평가는 "추천된 글이 사용자가 작성 중인 글의 근거로 실제로 쓸 수 있는가"를 기준으로 판정했다. 단어가 일부 겹치는 것만으로는 관련 글로 보지 않았다.

| 판정 | 기준 | 예시 |
|---|---|---|
| 관련 | 제목, 본문, 태그 중 핵심 주제가 평가 쿼리와 직접 연결되고, 초안 작성 근거로 사용할 수 있음 | `GitHub Actions 배포 자동화` 쿼리에서 workflow, secrets, CI/CD, deploy를 다루는 글 |
| 부분 관련 | 큰 주제는 맞지만 세부 조건이 어긋나거나 근거로 쓰기에는 약함 | `오늘 날씨 브리핑` 쿼리에서 날씨 글은 맞지만 지역이 다른 글 |
| 비관련 | 같은 카테고리이거나 일부 단어만 겹치고, 작성 중인 글의 근거로 사용하지 않음 | `GitHub` 쿼리에서 단순 회고나 게임 리뷰 글 |

최신 온라인 검색 평가의 6개 케이스는 평가 스크립트에 관련 게시글 id 목록을 고정해 두었다. 기존 Spring 내부 5케이스는 서비스 내부 평가 기준을 사용하며, 최신 최종값과 직접 비교하지 않고 참고값으로 둔다. 시나리오 입출력 평가는 추천 제목과 초안 내용을 사람이 읽고 `pass`, `partial`, `fail`로 판정한다.
...
최종 튜닝은 6개 검색 케이스를 반복해서 사용했기 때문에 과적합 위험이 있다. 이 보고서는 해당 위험을 전제로 두고, 실험을 다음처럼 분리했다.

| 위험 | 대응 |
|---|---|
| 같은 6개 케이스에 맞춰 검색 파라미터가 과하게 조정될 수 있음 | 오프라인 실험은 후보 선별로만 사용하고, 최종 채택은 실제 API 경로의 온라인 후보 검증으로 결정 |
| 평균 점수만 높이고 실제 화면 품질은 낮아질 수 있음 | 대표 입력을 넣어 추천 제목과 초안 내용을 확인하는 시나리오 입출력 평가를 별도로 수행 |
| 특정 주제에만 유리한 조합이 선택될 수 있음 | RAG/LLM, GitHub Actions, Kubernetes/EKS, Redis, React, Airflow처럼 서로 다른 기술 주제를 포함 |
| 6케이스 규모 자체가 작음 | 다음 개선 작업에 holdout 평가셋과 수동 라벨 확장을 포함 |

따라서 현재 점수는 "모든 게시글 검색 상황의 일반 성능"이 아니라, Project Alpha의 현재 데이터와 대표 사용 시나리오에서 검증한 운영 기준선으로 해석한다.
...
검색 점수는 "좋은 근거를 가져왔는가"를 본다. RAGAS는 그 근거를 사용해 만든 답변이나 초안이 근거에 충실한지, 질문과 맞는지 보는 보조 평가다. 이 프로젝트에서는 RAGAS를 최종 검색 조합 선택의 주 지표로 쓰지 않고, 생성 결과의 근거성과 응답 관련성을 점검하는 보조 지표로 사용했다.
...
| 관찰 | 해석 | 다음 개선 |
|---|---|---|
| 첫 실행에서 `Context Recall = 1.0000`이지만 `Answer Relevancy = 0.3589` | 근거는 충분히 포함됐지만 생성 초안이 질문 의도에 덜 맞았다. 검색보다 초안 프롬프트와 근거 압축 방식의 문제다. | 초안 생성 프롬프트에서 사용자 제목/본문 의도를 더 강하게 고정 |
| 두 번째 이후 `Context Precision = 0.9667`로 개선 | 근거 후보가 더 정제되면서 불필요한 context가 줄었다. | RAG 검색의 source filtering 유지 |
| `Faithfulness`는 0.8대 유지 | 생성 내용이 근거에서 크게 벗어나지는 않는다. | 사실성 요구가 높은 기능에는 MCP fact check나 별도 검증 단계 연결 |
| `Semantic Similarity = 0.9403` | 기준 답변과 의미 유사도는 높다. | 답변 관련성 개선을 위해 reranker와 prompt를 함께 튜닝 |

RAGAS 결과의 한계도 명확하다. Rows가 5개라 검색 랭킹 전체를 대표하지 않고, RAGAS는 검색 결과 순위 자체보다 생성 답변 품질을 평가한다. 따라서 RAGAS는 `MRR@5`, `NDCG@5`, `Precision@5`를 대체하지 않고, RAG 초안 생성의 보조 평가로 사용한다.
...
실제 현재 저장된 청크 수는 `post_embedding_chunks = 5419`개이고, 게시글당 평균 `4.14`청크다. 단순 추정의 `1200/180 = 5178`개보다 많은 이유는 실제 구현이 문단 경계를 최대한 보존하면서 새 청크를 만들기 때문이다.
...
점수표는 실제 사용자 경험을 일부만 설명한다. 그래서 대표 입력과 실제 출력도 함께 본다.
...
예를 들어 유사도가 일정 값보다 낮으면 결과에서 제외한다. 단, threshold만으로는 높은 점수의 오탐을 처리하지 못한다. 점수는 높지만 주제가 어긋나는 경우가 있기 때문이다.
